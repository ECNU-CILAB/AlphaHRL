...
class LinearAlphaPool(AlphaPoolBase, metaclass=ABCMeta):
    def __init__(
        ...
        self._best_json_dict: Optional[Dict[str, Any]] = None
        self._best_snapshot_version: int = 0
    ...
    def try_new_expr(self, expr: Expression) -> float:
        # step 1: calculate the IC and mutual information
        ...
        if ic_ret is None or ic_mut is None or np.isnan(ic_ret) or np.isnan(ic_mut).any():
            # Invalid expression
            return 0.
        # step 3: check in the failed cache
        if str(expr) in self._failure_cache:
            return self.best_obj

        self.eval_cnt += 1
        # step 4: temporarily add the factor(beyond the limit)
        old_pool: List[Expression] = self.exprs[:self.size]
        self._add_factor(expr, ic_ret, ic_mut)
        if self.size > 1:
            # step 5: optimize weights
            new_weights = self.optimize()
            worst_idx = None
            # step 6: remove the worst factor if it is over the limit
    ...
    def _maybe_update_best(self, ic: float, obj: float) -> bool:
        ...
        if not math.isclose(float(self.best_obj), float(self.best_ic_ret), rel_tol=1e-12, abs_tol=1e-12):
            best_json_dict["best_obj"] = float(self.best_obj)
        self._best_json_dict = best_json_dict
        self._best_snapshot_version += 1
        return True

    @property
    def best_snapshot_version(self) -> int:
        return self._best_snapshot_version

    def best_to_json_dict(self) -> Dict[str, Any]:
        if self._best_json_dict is None:
            return self.to_json_dict()
        return dict(self._best_json_dict)
