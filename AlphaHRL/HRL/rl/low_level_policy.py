...
class TransformerSharedNet(BaseFeaturesExtractor):
    def __init__(
        ...
        self._token_emb = nn.Embedding(n_actions + 1, d_model, 0)
...
class Decoder(BaseFeaturesExtractor):
    ...
    def forward(self, obs: Tensor) -> Tensor:
        ...
        obs = torch.cat((begins, obs.type(torch.long)), dim=1)
        pad_mask = obs == 0
        res = self._token_emb(obs)
        res = self._pos_enc(res)
        res = self._decoder(res, src_key_padding_mask=pad_mask)
        return res.mean(dim=1)
