...
_DEFAULT_QLIB_DATA_PATH = "/home/liuyu/.qlib/AlphaGen_qlib_data/qlib_data/cn_data_rolling/"
...
class StockData:
    ...
    def _load_exprs(self, exprs: Union[str, List[str]]) -> pd.DataFrame:
        ...
        if cal[end_index] != pd.Timestamp(self._end_time):
            end_index -= 1
        real_end_time = cal[min(len(cal) - 1, end_index + self.max_future_days)]

        return (QlibDataLoader(config=exprs)
                .load(self._instrument, real_start_time, real_end_time))
